...
for y, x in t.nodes():
    ...
    if Table[y][x] == 0:
        ...
        for com in itertools.combinations(edges, 2):
            c = Cnf()
            for edge in com:
                c |= - edge
            exp &= c
    elif Table[y][x] == 2:
        for com in itertools.combinations(edges, 3):
            c = c1 = Cnf()
            for edge in com:
                c  |= - edge
                c1 |= edge
        ...
        assert Table[y][x] == 5, 'format number error'

# You have to draw lines between the dots to form without crossings or branches
# Duong Manh
# 11/11/2017
for node in g.nodes():
    ...
    for com in adje:
        c = Cnf()
        for edge in [e for e in adje if e != edge]:
            c |= edge
        exp &= - com | c

    for com in itertools.combinations(adje, 3):
        c = Cnf()
        for edge in com:
            c |= - edge
        exp &= c

# You have to draw lines between the dots to form a single loop
# Duong Manh
# 11/11/2017
for node1, node2 in g.edges():
    ...
    #     c2 = cell2['variable']
    #
    #     # edgeMerge <=> (cell1 <=> ~ cell2)
    #     exp &= - edgeMerge | - c1 | - c2
    #     exp &= - edgeMerge |   c1 |   c2
    #     exp &=   edgeMerge | - c1 |   c2
    #     exp &=   edgeMerge |   c1 | - c2
    # if cell1 == None and cell2 != None:
    #     #cell2 = cell2['variable']
    #     exp &= - edgeMerge | cell2['variable']
    #     exp &=   edgeMerge | - cell2['variable']
    # if cell1 != None and cell2 == None:
    #     #cell1 = cell1['variable']
    #     exp &= - edgeMerge | cell1['variable']
    #     exp &=   edgeMerge | - cell1['variable']
